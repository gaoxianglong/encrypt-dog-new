...
- **WHEN** 用户运行 `java -jar encryptdog-2.0.5.jar -h` 查看启动输出
...
- **WHEN** 用户运行 `java -jar encryptdog-2.0.5.jar -h` 查看帮助
- **THEN** usage 首行以 `Usage: encryptdog` 开头，输出中不出现 `encrypt-dog`

### Requirement: 构建产物命名

Maven 构建 SHALL 产出 jar `encryptdog-<version>.jar`（如 `encryptdog-2.0.5.jar`），macOS 打包 SHALL 产出 `EncryptDog.app` 与 `EncryptDog-<version>.dmg`；SHALL NOT 产出 `dog-<version>.jar` 命名的 jar。

#### Scenario: 构建产物名称

- **WHEN** 执行 `mvn package` 与 `build-mac.sh` 打包
- **THEN** `target/` 下存在 `encryptdog-2.0.5.jar` 且不存在 `dog-2.0.5.jar`；`dist/` 下存在 `EncryptDog.app` 与 `EncryptDog-2.0.5.dmg`
