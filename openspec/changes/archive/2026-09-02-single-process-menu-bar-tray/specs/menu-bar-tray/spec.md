...
GUI 模式启动时，系统 SHALL 以单进程方式在 macOS 菜单栏挂载应用图标（`logo.png` 品牌标志资源，与标题栏同源，22pt 与 44pt@2x 双档多分辨率呈现，等比缩放不拉伸变形），不引入独立启动参数、守护进程或第二个 JVM；终端模式 SHALL NOT 挂载菜单栏图标。系统托盘不可用或图标资源缺失时 SHALL 静默降级为无托盘运行，GUI 其余功能正常，且该场景下窗口 ✕ SHALL 保持退出进程语义。

#### Scenario: GUI 模式启动挂载图标

- **WHEN** 用户以 `--gui` 方式启动加密狗
- **THEN** macOS 菜单栏出现应用图标（清晰不模糊、与 Dock 图标同源），tooltip 显示 "EncryptDog"，进程为单进程形态
...
托盘菜单 SHALL 在空闲与干活中两个状态呈现不同内容，菜单与 tooltip 文案 SHALL 全部为英文、SHALL NOT 使用 emoji 或装饰符号。空闲态菜单项依次为 Show、Reveal last output、Quit，tooltip 为 "EncryptDog"。干活态菜单项依次为 Show、状态行、Quit——状态行为禁用（不可点击）的菜单项，文案格式 "Encrypting {done}/{total} · {pct}%"，其中 done 为已完成处理文件数（含失败文件）、total 为总文件数、pct 为按字节加权的整体进度百分比；tooltip 与状态行文案同步。执行开始 SHALL 立即切入干活态，执行结束（含全部成功、部分失败与执行异常）SHALL 立即切回空闲态。

#### Scenario: 空闲态菜单

- **WHEN** 未在执行任务时用户点击菜单栏图标
- **THEN** 弹出菜单依次为 Show、Reveal last output、Quit 三项，tooltip 为 "EncryptDog"

#### Scenario: 干活态菜单与进度状态行

- **WHEN** 任务执行中用户点击菜单栏图标
- **THEN** 弹出菜单为 Show、状态行、Quit；状态行禁用并以 "Encrypting 3/12 · 41%" 形式展示当前进度，tooltip 同步展示同一进度文案
